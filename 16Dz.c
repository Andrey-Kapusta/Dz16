...
double* create_array_after_last_negative(int* ptr_array2, int size, int* ptr_array4, int size4);
void Task_11();
void Task_14();
int Task_16();
int Task_16_dz();
...
double* create_array_after_last_negative(int* ptr_array2, int size, int *ptr_array4, int* size4) {
    int last_negative_index = 0; // Индекс последнего отрицательного элемента. Изначально предполагаем, что нет отрицательных
    ...
    for (int i = last_negative_index+1; i < size; i++) {
        ptr_array4 = insert(ptr_array4, &size4, 0, ptr_array2[i]);
    }
